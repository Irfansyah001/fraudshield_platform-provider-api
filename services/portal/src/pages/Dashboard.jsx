...
export default function Dashboard() {
  ...
  // Debug: Tampilkan status admin di console
  console.log('[Dashboard] User:', user?.email, 'Role:', user?.role, 'isAdmin:', isAdmin)
  ...
  return (
    <div className="min-h-screen bg-[#0f0f23]">
      ...
      >
          ...
          {navigation.map((item) => (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                isActive(item.href)
                  ? 'bg-linear-to-r from-[#667eea]/20 to-[#764ba2]/20 text-white border border-[#667eea]/30 shadow-lg shadow-purple-500/10'
                  : 'text-gray-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              <item.icon className={`w-5 h-5 ${isActive(item.href) ? 'text-[#667eea]' : ''}`} />
              {item.name}
              {isActive(item.href) && (
                <ChevronRight className="w-4 h-4 ml-auto text-[#667eea]" />
              )}
            </Link>
          ))}
        ...
        {/* User Info */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-white/5 bg-[#0f0f23]/50">
          <div className="flex items-center gap-3 mb-4">
            <div className={`w-11 h-11 rounded-full flex items-center justify-center ring-2 ${
              isAdmin 
                ? 'bg-linear-to-br from-amber-500 to-orange-500 ring-amber-500/20' 
                : 'bg-linear-to-br from-[#667eea] to-[#764ba2] ring-[#667eea]/20'
            }`}>
